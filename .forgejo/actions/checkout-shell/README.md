...
- The action uses POSIX `sh` only — `set -eu`, no pipes, no bashisms.
  Works in minimal Alpine containers (`tcwlab/trivy`) that drop bash,
  and on Ubuntu host runners (where `/bin/sh` is `dash` and rejects
  `set -o pipefail`).
